...
title: Hermes
shortTitle: Hermes
description: Hermes。本文属于Codex CLI专题，面向国内用户梳理 Code80 接入与配置步骤。
date: 2026-08-27
category: Codex CLI
tag:
  - Code80
...
* 支持 Nous Portal、OpenRouter、OpenAI 等多种供应商，也支持**任意 OpenAI / Anthropic 兼容端点** ，因此可以直接接入 Code80。
...
3. 首次启动会进入设置向导，供应商选择部分可以先跳过，下面我们通过 CC-Switch 配置 Code80 渠道。

## 配置 Code80 渠道

1. 查看 [CC Switch下载](/code80/ccswitch/1-common) 一节的内容，下载并安装 CC-Switch 到本地，安装并打开软件

2. 上方配置项选择到 `Hermes`，然后点击 `添加供应商` 按钮

![步骤截图](/images/code80/Advanced/Hermes/01.webp)

3. 进行多项配置：

* 在 `预设供应商` 中选择 `Code80`
* 在 `供应商标识` 中填写名称（只能使用小写字母、数字和连字符），比如 code80
* `API 模式` 保持 `Anthropic Messages`，`API 端点` 保持 `https://code.ai80.vip`
* 在 `API Key` 中填入 [创建API令牌](/code80/register/4-token) 一节中你创建的Key

::: warning 重要
**目前推荐在 Hermes 使用的分组：**

* **Claude：[aws-q分组](/code80/token/2-group)、[cc-sale分组](/code80/token/2-group)、[claude-officially分组](/code80/token/2-group)、cc-expensive分组**

**请您创建正确分组的APIKEY后填入**
:::

* 在 `模型列表` 中配置ApiKey对应分组下正确的模型名，每个模型需要填写 `模型 ID` 和 `显示名称`（`高级选项` 保持默认即可），每个分组下的模型可在 [令牌分组介绍](/code80/token/2-group) 一节中查询。
...
![步骤截图](/images/code80/Advanced/Hermes/02.webp)![步骤截图](/images/code80/Advanced/Hermes/03.webp)

4. 在界面中选择刚配置好的 Code80 渠道，点击 `启用` 按钮，即可完成渠道启用

![步骤截图](/images/code80/Advanced/Hermes/04.webp)
...
2. 输入 `/model` 命令，在模型选择器中能看到刚配置的 Code80 渠道及模型列表，选中想要的模型回车确认；后续切换渠道时回到 CC-Switch 一键切换即可

![步骤截图](/images/code80/Advanced/Hermes/05.webp)

3. 随便发送一句话，能正常收到回复即表示配置成功，开始愉快的对话吧~

![步骤截图](/images/code80/Advanced/Hermes/06.webp)
