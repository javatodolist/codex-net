...
shortTitle: 缓存优化
description: 介绍如何在 Code80 中完成「Claude Code 缓存优化代理」。步骤与原文一致，接口与控制台均指向 Code80。
layout: doc
---
...
Code80 和 `claude-code-cache-fix` 的整套配置。不要把 Windows 原生
Claude Code 与 WSL 代理混在一起使用，否则很容易出现路径、环境变量、
后台服务和本地端口不一致的问题。
:::

::: tip 第三方工具提醒
`claude-code-cache-fix` 不是 Code80 官方维护工具。它会经过本机 API 请求，
安装前请自行审查源码、依赖和配置方式。
:::
...
推荐把你的系统环境、Code80 Endpoint 和 GitHub 项目链接一起发给 AI，
让 AI 按你的机器生成命令。
...
要求：
```
1. 我使用 Code80，upstream 必须是 https://code.ai80.vip
2. Claude Code 的 ANTHROPIC_BASE_URL 应指向本地代理 http://127.0.0.1:9801
3. 保留 ANTHROPIC_AUTH_TOKEN，用我的 Code80 CC 分组令牌替换
...
CACHE_FIX_PROXY_UPSTREAM=https://code.ai80.vip cache-fix-proxy server
```

## 配置 Claude Code

代理启动后，将 Claude Code 的 `settings.json` 中 `ANTHROPIC_BASE_URL`
改为本地代理地址，`ANTHROPIC_AUTH_TOKEN` 继续填写你的 Code80
**CC** 分组令牌：
...
能正常进入 Claude Code 并收到回复，说明代理和 Code80 配置已经连通。
